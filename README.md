...
| 🕒 | **时间区间** | 可选按星期 + 起止时间运行（如工作日 09:00-18:00、跨夜 22:00-06:00），窗口外跳过检查；支持 UTC 或系统时区 |
...
3. **运行设置** → 调整检查间隔与「最近发现 / 检查历史」显示条数；顶部开关启动/停止监控；展开「时间区间」可按星期 + 起止时间限制监控只在窗口内运行（支持跨夜 22:00-06:00、UTC 或系统时区）
